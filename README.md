...
5. Run `npm run test` — everything must pass: 1125 vitest unit tests across 107 files, 187 solver validation tests in `solver_validation.ts`, the parallel-assembly equivalence suite, and 296 client logic checks (a few vitest tests self-skip where the TetGen/Gmsh binaries are absent, so the raw totals show a handful of skips). The run finishes with five drift guards; if one fails, the doc it names is stale, not the guard.
