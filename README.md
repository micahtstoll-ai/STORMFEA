...
5. Run `npm run test` — everything must pass: 1116 vitest unit tests across 105 files, 187 solver validation tests in `solver_validation.ts`, the parallel-assembly equivalence suite, and 289 client logic checks (a few vitest tests self-skip where the TetGen/Gmsh binaries are absent, so the raw totals show a handful of skips). The run finishes with five drift guards; if one fails, the doc it names is stale, not the guard.
