...
- **Tests must pass.** Run `npm run test` before opening a PR. The full suite covers 328 Vitest unit tests, 103 solver validation tests (patch tests, cantilever benchmarks, isotropic-limit and Hill-criterion checks, hole-in-plate Kt, weak-axis Bond rotation, two-region field equivalence), the parallel-assembly equivalence check, and 53 client-logic checks. A regression in any of these is a blocker. See [docs/METHODOLOGY.md](docs/METHODOLOGY.md#9-validation) for what the solver suite proves.
