...
- **#316** (invariant 4, C3D10 midsides on a genuinely CURVED surface) — now
  covered by `server/tests/unit/distance-curved-midside.test.ts`, which meshes a
  bored plate with the real Gmsh `-order 2` kernel (bore midsides land ON the
  cylinder, not on the chord). Measured worst boundary-midside distance 48.0
  microns vs an analytic sagitta of 39.4 microns at ~25 segments; corners still
  bit-exactly 0. Decision recorded, not retuned: the offset is immaterial to the
  wall/core classification (`computeWallFractions` moves by at most 3.6e-4, 18 of
  6292 elements, because the offset sits at the surface deep in the shell band,
  not at the `dist = tWall` interface). See the note under invariant 4.
