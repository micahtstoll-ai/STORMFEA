...
CLAUDE.md source: "Two-Region Material Model — Invariants" (items 1–8).
...
| 4 | The distance field is point-to-triangle (not nearest-node) so boundary triangles between 3–6 mm don't alias a ~1.35 mm wall band; boundary nodes seed at exactly 0 | `server/solver/distance.ts` `pointTriangleDistance`, `computeNodeSurfaceDistances` / `computeNodeSurfaceDistancesAndNormals` | `server/tests/unit/two-region.test.ts` (distance-field cases); no dedicated node-vs-boundary-triangle-size regression | Medium — the "boundary nodes seed at exactly 0" sub-claim is not independently asserted anywhere findable; worth a direct unit test |
