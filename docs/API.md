...
| `forces[]` | array | `{ magnitude, direction[3], position[3], loadDistribution?, loadPatchDepthMm? }`. **`position` is accepted but NOT read by the solver** (issue #271) — every mode picks its nodes from `direction` alone, so moving it changes nothing. `loadDistribution` ∈ `"uniform"` (default, equal split over the extreme-face band) ｜ `"cosine_bearing"` (concentrated at a bolt bearing point) ｜ `"tapered_patch"` (raised-cosine patch on the extreme face, integrated as a consistent tributary-area traction — no hard patch rim, and the only mode whose loaded extent is scale-invariant; see issue #260). `loadPatchDepthMm` sets the `tapered_patch` contact depth in mm and is ignored by the other modes; omitted → 15% of the part's extent along `direction`, which is a judgement, not a measurement. |
